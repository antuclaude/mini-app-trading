...
# ============================================================
# TẦNG 1: LẤY DỮ LIỆU TỪ TWELVE DATA
# ============================================================
def lay_du_lieu(san_pham):
    if san_pham == "Vàng (Gold)":
        symbol = "XAU/USD"
    else:
        symbol = "BTC/USD"
...
# ============================================================
# TẦNG 2: RSI WILDER (chuẩn TradingView)
# ============================================================
...
# ============================================================
# TẦNG 4: XU HƯỚNG H1 (Higher High / Higher Low)
# ============================================================
def xac_dinh_xu_huong_H1(df_h1):
    ...
    n      = 3  # số nến mỗi bên để xác định fractal
...
# ============================================================
# TẦNG 5: FRACTAL SWING M15 (chuẩn SMC)
# ============================================================
...
# ============================================================
# TẦNG 6: VÙNG GOLDEN POCKET (61.8% - 78.6%)
# ============================================================
def tinh_vung_golden_pocket(swing):
    ...
    if loai == "BUY":
        gp_high = A - chieu_cao * 0.618   # 61.8% retracement
        gp_low  = A - chieu_cao * 0.786   # 78.6% retracement
    else:
...
# ============================================================
# TẦNG 7: XÁC NHẬN CHoCH M15
# ============================================================
def xac_nhan_choch(df, loai, gp_low, gp_high):
    """
    CHoCH = Nến đóng cửa phá qua swing high/low gần nhất
    trong khi giá đang trong/gần vùng Golden Pocket
    """
    ...
    # Nới lỏng vùng GP thêm 0.3% để tránh bỏ sót tín hiệu
    buffer = (gp_high - gp_low) * 0.3

    if loai == "BUY":
        trong_vung = lows[-3:].min() <= (gp_high + buffer) and gia >= (gp_low - buffer)
        if not trong_vung:
            return False, f"⏳ Chờ giá về vùng GP: {gp_low} – {gp_high}"
        # Nến xanh mạnh phá HIGH nến trước
        ...
        trong_vung = highs[-3:].max() >= (gp_low - buffer) and gia <= (gp_high + buffer)
        if not trong_vung:
...
def kiem_tra_phan_ky(df, loai, lookback=30):
    if len(df) < lookback:
        return False, "Chưa đủ dữ liệu RSI"
    ...
    rsi_now = rsi[-1]
    if np.isnan(rsi_now):
        return False, "RSI chưa tính được"
    ...
    mo_ta = (
        "🔥 RSI Divergence xác nhận!" if co_phan_ky
        else f"RSI: {round(rsi_now, 1)} — Chưa có phân kỳ"
    )
    return co_phan_ky, mo_ta, round(rsi_now, 1)

# ============================================================
# TẦNG 9: SL ĐỘNG (ATR) + TP THEO R:R THỰC TẾ
# ============================================================
def tinh_quan_ly_lenh(df, loai, gp_low, gp_high, san_pham):
    ...
    # Buffer tối thiểu theo sản phẩm
    if san_pham == "Vàng (Gold)":
        buffer_min = 1.5
    else:
        buffer_min = 200.0

    # SL dựa theo ATR + cấu trúc
    buffer_atr = max(atr * 1.2, buffer_min)

    if loai == "BUY":
        # SL = đáy thấp nhất 5 nến gần nhất - ATR buffer
        sl = round(min(lows[-5:]) - buffer_atr, 2)
        kc_sl = gia - sl
        tp1   = round(gia + kc_sl * 2.0, 2)   # R:R 1:2
        tp2   = round(gia + kc_sl * 3.0, 2)   # R:R 1:3
    else:
        # SL = đỉnh cao nhất 5 nến gần nhất + ATR buffer
...
# ============================================================
# TẦNG 10: ĐIỂM CONFLUENCE + WINRATE
# ============================================================
def tinh_confluence(xu_huong, trong_vung, co_choch, co_phan_ky, rsi_val, loai):
    ...
    # 3. CHoCH (quan trọng nhất = 2 điểm)
    if co_choch:
        diem += 2
    ...
    if co_phan_ky:
        diem += 1
        chi_tiet.append(f"✅ RSI Divergence ({rsi_val}) — xác nhận đảo chiều")
    else:
        chi_tiet.append(f"➖ RSI {rsi_val} — chưa có phân kỳ")

    # 5. RSI cực trị
    if loai == "BUY" and rsi_val < 35:
        diem += 1
        chi_tiet.append(f"✅ RSI quá bán ({rsi_val}) — hỗ trợ BUY")
    elif loai == "SELL" and rsi_val > 65:
        diem += 1
...
def lay_du_lieu_va_phan_tich(san_pham, khung_thoi_gian="15m"):
    # 1. Lấy dữ liệu
    df_m15, df_h1 = lay_du_lieu(san_pham)
    ...
    # 3. Tính RSI M15 (luôn hiển thị dù sideway)
    ...
    # Nếu sideway → trả về sớm nhưng vẫn có RSI
    if xu_huong == "SIDEWAY":
        return {
            "loi": None,
    ...
    # 7. RSI Divergence
    kq_rsi = kiem_tra_phan_ky(df_m15, loai)
    co_phan_ky = kq_rsi[0]
    mo_ta_rsi  = kq_rsi[1]
    rsi_val    = kq_rsi[2]
    ...
    # 10. Quản lý lệnh (chỉ tính khi đủ điều kiện)
    quan_ly = None
...
🏆 <b>Winrate:</b> {winrate}
💡 Chốt 50% tại TP1, dời SL về hoà vốn, để 50% chạy TP2
        """.strip()
